...
public:
    ...
    DWORD Classify(const VxVector &v) const
    {
        ...
        else if (GetFarPlane().Classify(v) > 0.0f)
            flags |= VXCLIP_BACK; // the vertex is fully off back
        if (GetLeftPlane().Classify(v) > 0.0f)
            flags |= VXCLIP_LEFT; // the vertex is fully off left
        else if (GetRightPlane().Classify(v) > 0.0f)
            flags |= VXCLIP_RIGHT; // the vertex is fully off right
        if (GetBottomPlane().Classify(v) > 0.0f)
            flags |= VXCLIP_BOTTOM; // the vertex is fully off left
        else if (GetUpPlane().Classify(v) > 0.0f)
            flags |= VXCLIP_TOP; // the vertex is fully off right
    ...
    BOOL IsInside(const VxVector &v) const
    {
        // Classification of the vertex to the 6 planes
        if (GetNearPlane().Classify(v) > 0.0f)
            return FALSE; // the vertex is fully off near
        if (GetFarPlane().Classify(v) > 0.0f)
            return FALSE; // the vertex is fully off back
        if (GetLeftPlane().Classify(v) > 0.0f)
            return FALSE; // the vertex is fully off left
        if (GetRightPlane().Classify(v) > 0.0f)
            return FALSE; // the vertex is fully off right
        if (GetBottomPlane().Classify(v) > 0.0f)
            return FALSE; // the vertex is fully off left
        if (GetUpPlane().Classify(v) > 0.0f)
            return FALSE; // the vertex is fully off right
