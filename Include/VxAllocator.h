...
public:
	...
	void    ResetPage();   
	struct CFreeMemoryUnit;
	CFreeMemoryUnit* CheckAllUnit(CFreeMemoryUnit* iStart);
#endif
...
public:
	...
	enum {
		PAGESIZE		= 262144,	// Pages size : directly related to the :19 in the MemoryUnit
		PAGEBITS		= 7,
...
private:
	...
	struct CMemoryUnit
	{
		...
		XDWORD			m_PrevSize:19;		// Size of previous element or size of the free block
		XDWORD			m_Size:5;			// Size of the element in DWORD so limited to 64 (and offset to the next) or size of the previous allocated unit
