...
See Also: VxBigFile
*************************************************************/
class VX_EXPORT VxSubFile : public VxStream
{
public:
    VxSubFile() : m_File(NULL), m_Start(0), m_Size(0) {}
    void Set(VxFile *iBigFile, XDWORD iOffset, XDWORD iSize);

    // Virtual overridings
    virtual XDWORD Position();
    virtual XDWORD Size();
    virtual XBOOL IsValid();
    virtual XBOOL Seek(int iOffset, SeekMode iOrigin);
    virtual XDWORD Read(void *oBuffer, XDWORD iSize);
